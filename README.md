...
Recent releases:
- **v0.4.11** (May 8, 2026): Hardware-key completion across every reference implementation. The `OSGT-HW-P256-v1` suite now works in the Python core, the Rust workspace, and the public browser inspector, with a hardware tutorial fixture and P-256 ECDH backed by a pinned vendored implementation in the viewer. Main has moved further since the tag: the live registry deployment path now has Compose/Caddy production routing, shared operator-token enforcement in both registry implementations, and Python-to-Rust SQLite migration tooling for the Axum registry.
...
- Shipping Oversight past the public-launch gate: an Outlook add-in as the first ecosystem integration, Rust Axum registry burn-in after the Python-to-Rust migration tooling landed, and the hardware `PivKeyProvider` binding for YubiKey, Nitrokey, and OnlyKey.
